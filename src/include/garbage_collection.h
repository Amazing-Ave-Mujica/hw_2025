...
struct GarbageAllocator{
            ...
            for (int r2 = 0; r2 < m_; r2++) {
                ...
                for (int c1 = 0; c1 < m_; c1++) {
                    for (int c2 = 0; c2 < m_; c2++) {
                        if (c1 == c2) {continue;}
                        db delta = -sim_[r1][c1] - sim_[r2][c2] + sim_[r1][c2] + sim_[r2][c1];
                        if(delta>0){swaps_.emplace_back(delta, r1, c1, r2, c2);}
                    }
                }
            }
        }
    ...
    // 进行最多 K 次操作，使总相似度最大
    auto maximize_similarity(Matrix<int>&allo,Matrix<std::vector<int>>&pos)->std::vector<std::pair<int,int>> {//NOLINT
...
返回值：pair<int,int>
*/
    auto garbage_collection(const std::vector<std::tuple<int,int,int>>&info)->std::vector<std::pair<int,int>>{//NOLINT
    ...
    auto garbage_collection(Matrix<std::vector<int>>pos)->std::vector<std::pair<int,int>>{//NOLINT
        Matrix<int>allo;
        for(int i=0;i<m_;i++){
            for(int j=0;j<=m_;j++){
