...
public:
    ...
    ResourceAllocator(int m, int n, int v, int l, const std::vector<int>& r, 
                      const std::vector<std::vector<double>>& alpha, const std::vector<double>& beta)
        : m_(m), n_(n), v_(v), l_(l), r_(r), alpha_(alpha), beta_(beta), 
          best_penalty_(std::numeric_limits<double>::max()), rng_(std::random_device{}()) {}
    ...
    auto SimulatedAnnealing(double T, double coolingRate, int maxIter) -> void {
        ...
        for (int iter = 0; iter < maxIter; ++iter) {
            ...
            int j1 = dist_n(rng_);
            int j2 = dist_n(rng_);
            while (j1 == j2) {j2 = dist_n(rng_);}

            // **随机选择资源 i**
            int i = dist_m(rng_);

            // **交换 i 资源的部分数量**
            int delta = std::uniform_int_distribution<int>(0, std::min(x[i][j1], v_ - x[i][j2]))(rng_);
            ...
            // **接受新解的策略**
            if (e_new < e_cur || std::exp(-(e_new - e_cur) / T) > std::uniform_real_distribution<double>(0.0, 1.0)(rng_)) {
                x = x_new;
                e_cur = e_new;
