...
auto main() -> int {
    ...
    std::vector<std::vector<int>> timeslice_data(m, std::vector<int>(((t - 1) / TIME_SLICE_DIVISOR) + 1, 0));
    std::vector<std::vector<double>> alpha(m, std::vector<double>(m, 0.0)); // 资源混合惩罚系数
    ...
    for (int i = 0; i < m; i++) {
        ...
        max_allocate[i] = *std::max_element(timeslice_data[i].begin(), timeslice_data[i].end());
    }
    for(int i=0;i<m;i++){
        if(i<m-1){
            resource[i]=static_cast<int>(1.0*max_allocate[i]*(n*v)/std::accumulate(max_allocate.begin(),max_allocate.end(),0));
        }else{
            resource[i]=n*v-std::accumulate(resource.begin(),resource.end(),0);
        }
    }
